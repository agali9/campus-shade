...
# ASU Tempe campus bounds (you can adjust these)
ASU_BOUNDS = {
...
def filter_buildings():
    ...
    if not input_file.exists():
        print(f"Γ¥î Input file not found: {input_file}")
        print(f"   Please ensure you have downloaded the building data.")
        return
    
    print(f"≡ƒôé Loading buildings from: {input_file}")
    ...
    for feature in data['features']:
        ...
        # Get representative point (centroid approximation)
        if geom['type'] == 'Polygon':
            ...
            # Check if center is in ASU bounds
    ...
    print(f"\nΓ£à Saved filtered buildings to: {output_file}")
    print(f"\n≡ƒô¥ To use this file, update backend/src/api.py:")
    print(f"   Change line:")
